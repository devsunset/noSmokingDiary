...
class FastAPIClient {
  ...
  getDiary(diaryId) {
    return this.apiClient.get(`/diaries/${diaryId}`);
  }

  getDiaries(keyword) {
    return this.apiClient.get(`/diaries/search/?keyword=${keyword}&max_results=10`).then(({data}) => {
      return data;
    });
  }

  getUserDiaries() {
    return this.apiClient.get(`/diaries/my-diaries/`).then(({data}) => {
      return data;
    });
  }

  createDiary(title, content, selfcheck, submitter_id) {
    const diaryData = {
      title,
      content,
      selfcheck,
  ...
  deleteDiary(diaryId) {
    return this.apiClient.delete(`/diaries/${diaryId}`);
  }
}
